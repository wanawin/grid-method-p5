...
# ---- Streamlit Cloud file lookup helpers (repo-relative) ----
from pathlib import Path as _Path

def find_repo_file(candidate_names):
    """Return first existing file path in repo root (same dir as this script).
    Used to load bundled filter files on Streamlit Cloud without hardcoding /mnt/data.
    """
...
# Load filters (prefer sidebar uploads; fall back to repo-bundled files if present)
# Tip: on Streamlit Cloud, files committed to the repo live next to this script.
    ...
    try:
        if loserlist_uploader is not None:
            ...
            p = find_repo_file([
                ...
                'loserlist_filters15_FIXED (1).csv',
                'loserlist_filters15_FIXED.csv',
                'loserlist filters15.txt',
                'loserlist_filters15.txt',
            ])
            if p is None:
                raise FileNotFoundError('No bundled LoserList file found in repo. Upload one in the sidebar, or add it to the repo root.')
            loser_text = p.read_text(encoding='utf-8', errors='ignore')
    ...
    try:
        if batch10_uploader is not None:
            ...
            p = find_repo_file([
                ...
                'lottery_filters_batch10 (1).csv',
            ])
            if p is None:
                raise FileNotFoundError('No bundled Batch10 CSV found in repo. Upload one in the sidebar, or add it to the repo root.')
            batch_df = pd.read_csv(p, dtype=str).fillna("")
