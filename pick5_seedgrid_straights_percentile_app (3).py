...
# Stores per-source parsing repairs/notes for uploaded filter files
FILTER_PARSE_NOTES = {}
...
class FilterDef:
    """Filter definition loaded from CSV/TXT sources.

    This app ingests multiple filter libraries with slightly different
    column naming conventions. To stay compatible, the constructor
    accepts: fid/id, enabled/enabled_default, origin/source.

    Canonical attributes used internally:
      - fid, name, enabled, applicable_if, expression, origin
    """
    ...
    def __init__(
        ...
        if fid is None and id is not None:
            fid = id
        if enabled_default is not None:
            enabled = bool(enabled_default)
        if source is not None and not origin:
            origin = source
        self.fid = str(fid or "").strip()
        self.name = str(name or "").strip()
        self.enabled = bool(enabled)
        self.applicable_if = str(applicable_if or "").strip()
        self.expression = str(expression or "").strip()
        self.origin = str(origin or "").strip()

    @property
    def id(self) -> str:  # alias
        return self.fid

    @property
    def source(self) -> str:  # alias
        return self.origin

    @property
    def enabled_default(self) -> bool:  # alias
        return self.enabled



def load_filter_csv_text(text: str, source: str, max_rows: Optional[int] = None) -> List[FilterDef]:
    """Load filters from a CSV-ish text blob.

    This is intentionally forgiving because many filter files include unquoted commas
    inside the expression field, which breaks pandas' CSV parser.

    Strategy:
      * Parse with csv.reader
      * Use header length as the expected width
      * If a row has too many fields, merge extras into the last field
      * If a row has too few fields, pad with blanks

    We record any repairs in FILTER_PARSE_NOTES[source].
    """
    import csv
    import io
    ...
    reader = csv.reader(buf, delimiter=",", quotechar='"', escapechar='\\')
    rows = list(reader)

    if not rows:
        FILTER_PARSE_NOTES[source] = ["No rows found."]
        return []

    header = [h.strip() for h in rows[0]]
    expected = len(header)

    # If there's no useful header, fall back to the known 15-field schema
    if expected < 5 or ("expression" not in [h.lower() for h in header] and "name" not in [h.lower() for h in header]):
        header = ["id","name","enabled","applicable_if","expression"] + [f"Unnamed: {i}" for i in range(5, 15)]
        expected = len(header)
        notes.append("Header not detected; using default 15-field schema.")
        data_rows = rows
    else:
        data_rows = rows[1:]
    ...
    for line_no, row in enumerate(data_rows, start=2):
        if not row or all((c.strip()=="" for c in row)):
            continue
        if len(row) > expected:
            # Merge overflow into last field
            row = row[:expected-1] + [",".join(row[expected-1:])]
            notes.append(f"Line {line_no}: had extra fields; merged overflow into last column.")
        elif len(row) < expected:
            row = row + [""]*(expected-len(row))
            notes.append(f"Line {line_no}: had missing fields; padded with blanks.")
        cleaned.append(row)
    ...
    id_col = get_col('id','fid','filter_id')
    name_col = get_col('name')
    enabled_col = get_col('enabled')
    applicable_col = get_col('applicable_if','applicable')
    expr_col = get_col('expression','expr')

    missing = [n for n,v in [('id',id_col),('name',name_col),('enabled',enabled_col),('applicable_if',applicable_col),('expression',expr_col)] if v is None]
    if missing:
        FILTER_PARSE_NOTES[source] = notes + [f"Missing required columns: {', '.join(missing)}"]
        raise ValueError(f"Filter file is missing required columns: {', '.join(missing)}")
    ...
    for _, r in df.iterrows():
        ...
        enabled_val = str(r.get(enabled_col, 'TRUE')).strip().upper()
        enabled = enabled_val not in {'FALSE','0','NO','N'}
        applicable_if = str(r.get(applicable_col, '')).strip()
        expression = str(r.get(expr_col, '')).strip()
        if not expression or expression.lower() in {'nan','none'}:
            continue
        out.append(FilterDef(fid=fid, name=name or fid, enabled=enabled, applicable_if=applicable_if, expression=expression, origin=source))

    FILTER_PARSE_NOTES[source] = notes
    return out
...
def _get_filter_id_name(filt) -> tuple[str, str]:
    try:
        if isinstance(filt, dict):
            return str(filt.get('id') or filt.get('fid') or ''), str(filt.get('name') or '')
        return str(getattr(filt, 'fid', getattr(filt, 'id', ''))), str(getattr(filt, 'name', ''))
    except Exception:
        return '', ''


_LEADING_ZERO_INT_RE = re.compile(r"(?<![A-Za-z0-9_\.\"\'])0[0-9]+(?![A-Za-z0-9_])")

def sanitize_expression(expr: str) -> str:
    """Avoid SyntaxWarning/parse errors from numeric literals with leading zeros.

    Example: a filter might contain something like `combo_int == 01234`,
    which Python treats as invalid. We rewrite those tokens as int("01234").
    """
    if not expr:
        return expr
    def repl(m: re.Match) -> str:
        tok = m.group(0)
        return f'int("{tok}")'
    return _LEADING_ZERO_INT_RE.sub(repl, expr)


def safe_eval(expr: str, env: Dict[str, Any], filt=None, tracker=None) -> bool:
    if expr is None:
        return True
    expr = str(expr).strip()
    if expr == '' or expr.lower() in {'true', 't', '1', 'yes'}:
        return True
    if expr.lower() in {'false', 'f', '0', 'no'}:
        return False

    expr2 = sanitize_expression(expr)
    try:
        code = compile(expr2, '<filter>', 'eval')
        out = eval(code, {'__builtins__': {}}, env)
        return bool(out)
    except Exception as e:
        fid, name = _get_filter_id_name(filt)
        if tracker is not None:
            try:
                tracker.add(fid=fid, name=name, error=str(e), expression=expr)
            except Exception:
                pass
        return False
...
# Show any parse repairs we had to apply (helps you fix the source files)
for _src_key, _src_label in (("loserlist", "LoserList"), ("batch10", "Batch10")):
    _notes = FILTER_PARSE_NOTES.get(_src_key, [])
    if _notes:
        with st.expander(f"{_src_label} parse repairs ({len(_notes)})", expanded=False):
            st.write("\n".join(_notes[:300]))
...
], columns=["id", "name", "source", "winner_keep_rate", "avg_elim_rate", "apply_count"])
...
max_auto = min(100, len(ranked_filters))
if max_auto <= 0:
    apply_count = 0
    st.info("No ranked filters available for auto-apply at the current keep-rate threshold (or no filters loaded).")
else:
    apply_count = st.slider("Max filters to auto-apply", 0, max_auto, min(25, max_auto))

if apply_count <= 0 or df_perf.empty:
    to_apply_ids = set()
    filters_to_apply = []
    order = {}
else:
    # Some versions/sources may use 'fid' instead of 'id'
    id_col = 'id' if 'id' in df_perf.columns else ('fid' if 'fid' in df_perf.columns else None)
    if id_col is None:
        to_apply_ids = set()
        filters_to_apply = []
        order = {}
    else:
        _ids = df_perf.head(apply_count)[id_col].astype(str).tolist()
        to_apply_ids = set(_ids)
        filters_to_apply = [f for f in filters_all if str(getattr(f, 'fid', getattr(f, 'id', ''))) in to_apply_ids]

        # Preserve the ranked order from df_perf
        order = {fid: i for i, fid in enumerate(_ids)}
        filters_to_apply.sort(key=lambda f: order.get(str(getattr(f, 'fid', getattr(f, 'id', ''))), 999999))
