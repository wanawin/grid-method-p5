...
def extract_last_5digit_result(line: str) -> Optional[str]:
    """Return the last 5-digit token on the line."""
...
def load_filter_csv_text(text: str, source: str, max_rows: Optional[int] = None) -> List[FilterDef]:
    ...
    if not all(k in cmap for k in need):
        raise ValueError(f"Filter file missing required columns: {need}")
    out = []
    for i, r in df.iterrows():
        ...
        out.append(
            FilterDef(
                ...
                enabled_default=str(r[cmap["enabled"]]).strip().lower() == "true",
                applicable_if=normalize_expr(str(r[cmap["applicable_if"]]).strip() or "True"),
                expression=normalize_expr(str(r[cmap["expression"]]).strip() or "False"),
                source=source,
            )
        )
    return out
...
def evaluate_filters_on_pool(
    ...
    """Apply enabled filters in order. Returns (survivors, log rows)."""
    survivors = pool
    log = []

    # Prepare loserlist-aware resolver env
    for f in filters:
        ...
        for box in survivors:
            combo_digits = [int(x) for x in box]
            ...
            if not safe_eval(app_if, env):
                kept.append(box)
                continue
            # expression is ELIMINATE-if in these libraries
...
# Load filters from provided files on disk (uploaded earlier in this chat)
...
if use_batch10:
    try:
        ...
        # write to CSV in-memory then read with loader
        buff = io.StringIO()
...
survivors, log = evaluate_filters_on_pool(
    pool=ranked_pool,
