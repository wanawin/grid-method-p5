...
def load_filter_csv_text(text: str, source: str) -> List[FilterDef]:
    ...
    for _, r in df.iterrows():
        ...
        if not expr:
            continue
        # NOTE: FilterDef uses enabled_default as the field name.
        out.append(
            FilterDef(
                fid=fid,
                name=name,
...
def safe_eval(expr: str, env: Dict, *, filt=None, tracker: FailureTracker | None = None) -> bool:
    ...
    try:
        ...
        if tracker is not None and filt is not None:
            try:
                # Support both dict-like filters and FilterDef objects.
                if isinstance(filt, dict):
                    fid = str(filt.get("id", ""))
                    name = str(filt.get("name", ""))
                else:
                    fid = str(getattr(filt, "fid", ""))
                    name = str(getattr(filt, "name", ""))
                tracker.add(fid=fid, name=name, expression=str(expr), err=e)
            except Exception:
                pass
        return False
...
max_filters = min(100, len(ranked_filters))
if max_filters == 0:
    st.info("No filters met the current winner-keep threshold for auto-apply. (Try lowering the keep-rate slider or loading filter files.)")
    apply_count = 0
else:
...
if failure_tracker.has_any():
    ...
    with st.expander("Show failed filters (fix these in the filter files)", expanded=False):
        ...
        for row in failure_tracker.items:
            lines.append(f"{row.id}: {row.name} → {row.error_type}: {row.error_message}")
            if row.missing_vars:
                lines.append(f"  missing_var: {row.missing_vars}")
...
if len(final_pool) == 0:
    st.info("No combinations remain after filtering. Try reducing the number of auto-applied filters or lowering the keep-rate threshold.")
    st.stop()

straight_limit = st.slider(
    "How many box combos to expand into straights",
    1,
    len(final_pool),
    min(25, len(final_pool)),
)
