...
# Load filters (prefer sidebar uploads; fall back to bundled repo files if present)
filters_all: List[FilterDef] = []

from pathlib import Path


def _read_text_file_if_exists(paths: List[str]) -> Optional[str]:
    """Return file contents for the first existing path in `paths` (relative to repo root)."""
    for p in paths:
        if not p:
            continue
        pp = Path(p)
...
def _read_csv_df_if_exists(paths: List[str]) -> Optional[pd.DataFrame]:
    for p in paths:
        if not p:
            continue
        pp = Path(p)
        if pp.exists() and pp.is_file():
            return pd.read_csv(pp, dtype=str).fillna("")
    return None
...
if use_loser:
    try:
        if loserlist_uploader is not None:
            ...
            loser_text = _read_text_file_if_exists(DEFAULT_LOSER_CANDIDATES)
        if not loser_text:
            raise FileNotFoundError("No bundled LoserList file found. Upload it in the sidebar or commit it to the repo root.")
...
if use_batch10:
    try:
        if batch10_uploader is not None:
            ...
            batch_df = _read_csv_df_if_exists(DEFAULT_BATCH10_CANDIDATES)
        if batch_df is None:
            raise FileNotFoundError("No bundled Batch10 CSV found. Upload it in the sidebar or commit it to the repo root.")
...
# Build loser ctx for current run (needed for filter applicability)
...
# --- Combo lookup (box-level) ---
st.subheader("Combo lookup")
lookup_raw = st.text_input(
    "Enter a 5-digit combo to check (straight or box)",
    value="",
    help="We check by BOX (sorted digits). Example: 12345 or 53124 → box 12345.",
)
lookup_box = None
if lookup_raw.strip():
    digits_only = "".join(ch for ch in lookup_raw if ch.isdigit())
    if len(digits_only) != 5:
        st.warning("Enter exactly 5 digits (ignore spaces/commas).")
    else:
        lookup_digits = [int(ch) for ch in digits_only]
        lookup_box = box_key_from_digits(lookup_digits)
        st.write(f"Lookup box key: **{lookup_box}**")
        in_generated = lookup_box in set(boxes)
        in_topN = lookup_box in set(b for b, _ in ranked_scored)
        in_bins = lookup_box in set(b for b, _ in ranked_scored_bins)
        in_pool = lookup_box in set(ranked_pool)
        st.write({
            "in generated candidates": in_generated,
            "in Top-N ranked": in_topN,
            "in kept percentile bins": in_bins,
            "in pool entering filters": in_pool,
        })

# --- Manual 1-by-1 filter application (guided by aggression ranking) ---
st.subheader("Apply filters manually (1-by-1, guided)")

if "manual_pool" not in st.session_state:
    st.session_state["manual_pool"] = list(ranked_pool)
...
manual_pool = st.session_state.get("manual_pool", list(ranked_pool))
manual_log = st.session_state.get("manual_log", [])
manual_applied = st.session_state.get("manual_applied", [])

st.caption(f"Manual pool size (current): {len(manual_pool)}")
...
if len(ranked_filters) == 0:
    ...
    remaining_ranked = [p for p in ranked_filters if p.fid not in set(manual_applied)]
    if not remaining_ranked:
        st.info("All ranked filters have been applied in the manual run.")
    else:
        pick_labels = [
            f"{p.fid} — keep {p.keep_rate:.3f}, elim {p.avg_elim_rate:.3f} — {p.name}"
            for p in remaining_ranked[:250]
        ]
        choice = st.selectbox("Choose next filter to apply", pick_labels, index=0)
        ...
        with c2:
            undo = st.button("Undo last", disabled=(len(manual_log) == 0))
        with c3:
            st.write("Applied so far:", ", ".join(manual_applied[-12:]) if manual_applied else "—")

        if do_apply:
            fdef = filters_by_id.get(chosen_id)
            if fdef is None:
                st.error("Selected filter id not found in loaded filters.")
            else:
                before_n = len(manual_pool)
                survivors_m, _, failures_m = evaluate_filters_on_pool(
                    pool=manual_pool,
                    filters=[fdef],
                    seed=seed,
                    prev=prev,
                ...
                after_n = len(survivors_m)
                st.session_state["manual_pool"] = survivors_m
                st.session_state["manual_log"].append({
                    "filter_id": chosen_id,
                    "before": before_n,
                    "after": after_n,
                    "eliminated": before_n - after_n,
                    "had_parse_failures": failures_m.has_any(),
                })
                st.session_state["manual_applied"].append(chosen_id)

        if undo and manual_log:
            replay_ids = manual_applied[:-1]
            pool0 = list(ranked_pool)
            replay_filters = [filters_by_id[i] for i in replay_ids if i in filters_by_id]
            survivors_replay, _, _ = evaluate_filters_on_pool(
                pool=pool0,
            ...
            st.session_state["manual_pool"] = survivors_replay
            st.session_state["manual_applied"] = replay_ids
            st.session_state["manual_log"] = manual_log[:-1]

if st.session_state.get("manual_log"):
    st.write("Manual filter log")
    st.dataframe(pd.DataFrame(st.session_state["manual_log"]), use_container_width=True)

if lookup_box is not None:
...
to_apply_ids = set(df_perf.head(apply_count)["id"].tolist())
filters_to_apply = [f for f in filters_all if f.fid in to_apply_ids]

# Preserve the ranked order from df_perf
order = {fid: i for i, fid in enumerate(df_perf.head(apply_count)["id"].tolist())}
