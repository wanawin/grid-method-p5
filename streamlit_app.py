...
# Defaults so the section never crashes even before ranking is computed
id_col = None
apply_count = 0
to_apply_ids = set()

if len(ranked_filters) == 0:
    st.warning('No ranking computed yet. Click Run / Refresh filter ranking.')
else:
    id_col = 'id' if 'id' in ranked_filters.columns else ('fid' if 'fid' in ranked_filters.columns else None)
    if id_col is None:
        st.warning('Ranking table missing an id column (expected id or fid).')
    else:
        to_apply_ids = set(ranked_filters[id_col].astype(str).tolist())
        apply_count = len(to_apply_ids)
