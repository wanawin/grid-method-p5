...
# Auto-apply: choose up to N ranked filters (safest → more aggressive) and apply them to reach target
apply_count = 0
id_col = None
to_apply_ids = set()

if len(ranked_filters) == 0 or df_perf is None or len(df_perf) == 0:
    apply_count = 0
else:
...
order_list = df_perf.head(apply_count)[id_col].astype(str).tolist() if (id_col and apply_count > 0) else []
