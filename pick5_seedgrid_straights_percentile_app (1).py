...
def _find_repo_file(candidates):
    """Return the first existing file in the repo root (same dir as this script)."""
...
def _find_best_repo_file(preferred_names, glob_patterns):
    # Return the best-matching file in the repo directory, or None.
    ...
    for pat in glob_patterns:
        candidates.extend(list(app_dir.glob(pat)))
    ...
    def _score(p: Path):
        n = p.name.lower()
        return (
            0 if 'fixed' in n else 1,
            0 if 'no_loserlist' in n else 1,
            0 if n.endswith('.csv') else 1,
            -p.stat().st_mtime,
            -p.stat().st_size,
        )
...
def _load_default_loserlist_filters():
    preferred = [
    ...
    patterns = [
        'loserlist_filters*.csv',
        'loserlist_filters*.txt',
        '*loserlist*filters*.csv',
        '*loserlist*filters*.txt',
    ]

    p = _find_best_repo_file(preferred, patterns)
    if p and p.exists():
        try:
            if p.suffix.lower() == '.csv':
                return read_filters_csv(p)
            else:
                return parse_filter_lines(p.read_text(encoding='utf-8', errors='ignore').splitlines())
        except Exception:
            return []
    return []

def _load_default_batch10_filters():
    """Try to load Batch10 filters from the repo (optional)."""
    candidates = [
    ...
    return load_filter_csv_text(raw)
...
with st.sidebar:
    ...
    learn_opts = st.multiselect("Learning window candidates (transitions)", [120, 180, 240], default=[120])
    val_opts = st.multiselect("Validation window candidates (transitions)", [60, 90, 120], default=[60])
...
with colA:
    ...
    st.dataframe(df[["date", "result"]].head(10), width="stretch")
with colB:
...
if use_loser:
    try:
        if loserlist_uploader is not None:
            ...
            loser_text = _read_text_file_if_exists(repo_path)
            if loser_text is None:
                raise FileNotFoundError('No bundled LoserList filter file found in repo (or upload missing).')
    ...
    try:
        if batch10_uploader is not None:
            ...
            if repo_path is None:
                raise FileNotFoundError('No bundled Batch10 filter CSV found in repo (or upload missing).')
            batch_df = pd.read_csv(repo_path, dtype=str).fillna('')
    ...
    with st.spinner("Running auto calibration (walk-forward)..."):
        try:
            cal = calibrate_windows(
                ...
                learn_options=sorted(set(learn_opts)) or [120],
                val_options=sorted(set(val_opts)) or [60],
                gen_mode=gen_mode,
                gen_cap=int(gen_cap),
...
with st.expander("Show recent calibration rows"):
    st.dataframe(cal.details.tail(30), width="stretch")
...
st.dataframe(df_perf.head(50), width="stretch")
...
st.markdown('---')
st.subheader('Manual filter application (one-by-one)')
st.caption('Start from the post-percentile pool and apply filters one at a time, guided by the walk-forward ranking above.')

# Base pool for manual filtering: post-percentile keep, before any auto-applied filters
base_pool_for_manual = ranked_pool

# Initialize / reset manual state
if 'manual_pool' not in st.session_state:
    st.session_state.manual_pool = list(base_pool_for_manual)
if 'manual_applied' not in st.session_state:
    st.session_state.manual_applied = []

colA, colB, colC = st.columns([1, 1, 2])
with colA:
    if st.button('Reset manual pool'):
        st.session_state.manual_pool = list(base_pool_for_manual)
        st.session_state.manual_applied = []
with colB:
    st.write(f"Manual pool size: **{len(st.session_state.manual_pool)}**")
with colC:
    if st.session_state.manual_applied:
        st.write(f"Applied: {', '.join(st.session_state.manual_applied)}")

# Remaining filters (exclude ones already applied)
applied_ids = set(st.session_state.manual_applied)
remaining_filters = [f for f in filters_all if f.fid not in applied_ids]

# Build a guidance table for remaining filters using the walk-forward metrics we already computed
if not df_perf.empty and 'id' in df_perf.columns:
    df_remaining = df_perf[~df_perf['id'].astype(str).isin(applied_ids)].copy()
    # safest -> more aggressive: higher keep first, then lower elim
    df_remaining = df_remaining.sort_values(by=['winner_keep_rate', 'avg_elim_rate'], ascending=[False, True])
else:
    df_remaining = pd.DataFrame(columns=['id', 'name', 'source', 'winner_keep_rate', 'avg_elim_rate', 'apply_count'])

st.dataframe(df_remaining.head(100), width="stretch")

if remaining_filters:
    # Select a filter to apply
    id_to_filter = {f.fid: f for f in remaining_filters}
    # Prefer the guidance order if we have it
    ordered_ids = [i for i in df_remaining['id'].astype(str).tolist() if i in id_to_filter] if not df_remaining.empty else list(id_to_filter.keys())
    if not ordered_ids:
        ordered_ids = list(id_to_filter.keys())

    pick = st.selectbox('Choose next filter to apply', ordered_ids)
    if st.button('Apply selected filter'):
        f = id_to_filter.get(pick)
        if f is None:
            st.error('Could not find selected filter in remaining list.')
        else:
            before_n = len(st.session_state.manual_pool)
            new_pool, _log, _ft = evaluate_filters_on_pool(
                pool=st.session_state.manual_pool,
                filters=[f],
                seed=seed,
                prev=prev,
                prev2=prev2,
            ...
            st.session_state.manual_applied = st.session_state.manual_applied + [f.fid]
            after_n = len(st.session_state.manual_pool)
            st.success(f"Applied {f.fid}: {before_n} → {after_n}")
else:
    st.info('No remaining filters to apply.')

# Combo lookup (was it generated / where did it survive?)
st.markdown('---')
st.subheader('Combo lookup')
st.caption('Check whether a specific 5-digit combo exists in the generated / ranked / filtered pools.')

lookup = st.text_input('Enter a 5-digit combo to search (e.g., 01234)', value='')
lookup = ''.join([c for c in lookup.strip() if c.isdigit()])
if lookup:
    if len(lookup) != 5:
        ...
        gen_set = set(generated_boxes)
        ranked_set = set(ranked_pool)
        auto_set = set(survivors) if 'survivors' in locals() else set()
        manual_set = set(st.session_state.manual_pool) if 'manual_pool' in st.session_state else set()

        stages.append(('Generated boxes', lookup in gen_set))
        stages.append(('Post-ranking Top-N (then bins)', lookup in ranked_set))
        if auto_set:
            stages.append(('After auto-applied filters', lookup in auto_set))
        stages.append(('Manual pool (current)', lookup in manual_set))
...
st.dataframe(straight_df, width="stretch")
