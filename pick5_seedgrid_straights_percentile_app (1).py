...
with st.sidebar:
    ...
    # Backward-compatible aliases (older code used include_*)
    include_loser = use_loser
    include_batch10 = use_batch10
...
# Safe defaults so this section never crashes when ranking is empty
apply_count = 0
id_col = None
to_apply_ids = set()

if len(ranked_filters) == 0:
    st.info("Run ranking first (button above) to enable auto-apply.")
else:
...
order_list = df_perf.head(apply_count)[id_col].astype(str).tolist() if (id_col and apply_count > 0 and id_col in df_perf.columns) else []
